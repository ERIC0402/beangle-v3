 ...
 * </p>
 */
public class ComboPooledDataSource extends AbstractPoolBackedDataSource implements PooledDataSource, Serializable, Referenceable {
	final static MLogger logger = MLog.getLogger(ComboPooledDataSource.class);

	final static Set TO_STRING_IGNORE_PROPS = new HashSet(Arrays.asList(new String[] { "connection", "lastCheckinFailureDefaultUser", "lastCheckoutFailureDefaultUser",
			"lastConnectionTestFailureDefaultUser", "lastIdleTestFailureDefaultUser", "logWriter", "loginTimeout", "numBusyConnections", "numBusyConnectionsAllUsers", "numBusyConnectionsDefaultUser",
			"numConnections", "numConnectionsAllUsers", "numConnectionsDefaultUser", "numFailedCheckinsDefaultUser", "numFailedCheckoutsDefaultUser", "numFailedIdleTestsDefaultUser",
			"numIdleConnections", "numIdleConnectionsAllUsers", "numIdleConnectionsDefaultUser", "numUnclosedOrphanedConnections", "numUnclosedOrphanedConnectionsAllUsers",
			"numUnclosedOrphanedConnectionsDefaultUser", "numUserPools", "effectivePropertyCycleDefaultUser", "startTimeMillisDefaultUser", "statementCacheNumCheckedOutDefaultUser",
			"statementCacheNumCheckedOutStatementsAllUsers", "statementCacheNumConnectionsWithCachedStatementsAllUsers", "statementCacheNumConnectionsWithCachedStatementsDefaultUser",
			"statementCacheNumStatementsAllUsers", "statementCacheNumStatementsDefaultUser", "threadPoolSize", "threadPoolNumActiveThreads", "threadPoolNumIdleThreads", "threadPoolNumTasksPending",
			"threadPoolStackTraces", "threadPoolStatus", "overrideDefaultUser", "overrideDefaultPassword", "password", "reference", "upTimeMillisDefaultUser", "user", "userOverridesAsString",
				...
				if ("connectionPoolDataSource".equals(propName)) {
					if (val instanceof WrapperConnectionPoolDataSource) {
						...
						if (!(nested instanceof DriverManagerDataSource))
							throw new PropertyVetoException("ComboPooledDataSource requires that its unpooled DataSource " + " be set at all times, and that it be a"
									+ " com.mchange.v2.c3p0.DriverManagerDataSource. Bad: " + nested, evt);
					} else
						throw new PropertyVetoException("ComboPooledDataSource requires that its ConnectionPoolDataSource " + " be set at all times, and that it be a"
...
// now, referenceability happens exactly the same way it does for
 ...
 * ("checkoutTimeout"); // ("acquireIncrement"); // ("acquireRetryAttempts"); //
 * ("acquireRetryDelay"); // ("autoCommitOnClose"); //
 * ("connectionTesterClassName"); // ("forceIgnoreUnresolvedTransactions"); //
 * ("idleConnectionTestPeriod"); // ("initialPoolSize"); // ("maxIdleTime"); //
 * ("maxPoolSize"); // ("maxStatements"); // ("maxStatementsPerConnection"); //
 * ("minPoolSize"); // ("propertyCycle"); // ("breakAfterAcquireFailure"); //
 * ("testConnectionOnCheckout"); // ("testConnectionOnCheckin"); //
 * ("usesTraditionalReflectiveProxies"); // ("preferredTestQuery"); //
 * ("automaticTestTable"); // ("userOverridesAsString"); //
 * ("overrideDefaultUser"); // ("overrideDefaultPassword"); //
 * ("maxAdministrativeTaskTime"); // ("maxIdleTimeExcessConnections"); //
 * ("maxConnectionAge"); // ("connectionTesterClassName");
 * 
 * final static JavaBeanReferenceMaker referenceMaker = new
 * JavaBeanReferenceMaker();
 * 
 * static { referenceMaker.setFactoryClassName(
 * C3P0JavaBeanObjectFactory.class.getName() );
 * 
 ...
 * 
 * // identity token referenceMaker.addReferenceProperty("identityToken");
 * 
 * // shared properties (count: 1)
 * referenceMaker.addReferenceProperty("factoryClassLocation"); }
 * 
 * public Reference getReference() throws NamingException { synchronized ( this
 * ) { synchronized ( wcpds ) { synchronized( dmds ) {
 * //System.err.println("ComboPooledDataSource.getReference()!!!!"); //new
 * Exception("PRINT-STACK-TRACE").printStackTrace(); //javax.naming.Reference
 * out = referenceMaker.createReference( this ); //System.err.println(out);
 * //return out;
 * 
