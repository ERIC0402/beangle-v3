...
public class MenuServiceImpl extends BaseServiceImpl implements MenuService {
	...
	private OqlBuilder<Menu> buildMenuQuery(List<MenuProfile> profile, Group group) {
		...
		if (null != profile) {
			builder.where("menu.profile in (:profile)", profile);
		}
		return builder;
	}
	...
	public Menu getParentMenu(Menu menu) {
		if(StringUtils.isEmpty(menu.getCode()) || menu.getCode().indexOf(".") < 0){
			return null;
		}
		OqlBuilder<Menu> oql = OqlBuilder.from(Menu.class, "menu");
		String parentCode = menu.getCode().substring(0, menu.getCode().lastIndexOf("."));
		oql.where("menu.code = :code", parentCode);
		if(menu.getProfile() != null){
			oql.where("menu.profile.id = :profileId", menu.getProfile().getId());
		}
		List<Menu> menus = entityDao.search(oql);
		if(menus.size() > 0){
			return menus.get(0);
		}
		return null;
