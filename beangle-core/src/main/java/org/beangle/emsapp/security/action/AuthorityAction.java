...
public class AuthorityAction extends SecurityActionSupport {
	...
	public String edit() {
		...
		if (null != menuProfile) {
			List<MenuProfile> mps = new ArrayList<MenuProfile>();
			mps.add(menuProfile);
			...
			if (isAdmin()) {
				...
				menus = menuService.getMenus(mps, user);
			...
			Collection<Menu> aoMenus = menuService.getMenus(mps, (Group) ao, null);
	...
	public String save() {
		...
		Set<Resource> newResources = CollectUtils.newHashSet(entityDao.get(Resource.class, StrUtils.splitToLong(get("resourceId"))));
		// 管理员拥有的菜单权限和系统资源
		User user = entityDao.get(User.class, getUserId());
		...
		if (isAdmin()) {
			...
			menuProfiles.add(menuProfile);
			mngMenus = CollectUtils.newHashSet(menuService.getMenus(menuProfiles, (User) user));
		}
		for (final Menu m : mngMenus) {
			mngResources.addAll(m.getResources());
		}
		newResources.retainAll(mngResources);
		authorityService.authorize(group, newResources, mngResources);
		authorityManager.refreshGroupAuthorities(new GrantedAuthorityBean(group.getName()));

		Action redirect = Action.to(this).method("edit");
		redirect.param("group.id", group.getId()).param("menuProfileId", menuProfile.getId());
