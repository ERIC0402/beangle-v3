...
/**
 * @author beangle
 * @version $Id: DashboardHelper.java Nov 3, 2010 5:19:42 PM beangle $
 */
public class UserDashboardHelper {
	...
	private void populateMenus(User user) {
		List<MenuProfile> menuProfiles = menuService.getProfiles(user);
		...
		if (null != menuProfileId) {
			...
			List<Menu> menus = menuService.getMenus(menuProfiles, user);
			...
			for (Group group : myGroups) {
				groupMenusMap.put(group, menuService.getMenus(menuProfiles, group, Boolean.TRUE));
			}
