...
public class LoginUrlEntryPoint implements UrlEntryPoint, InitializingBean {
	...
	public void commence(ServletRequest request, ServletResponse response,
		...
		if(!("XMLHttpRequest".equals(httpRequest.getHeader("x-requested-with")) || StringUtils.isNotEmpty(httpRequest.getParameter("x-requested-with")))){
			String backurl=new UrlBuilder(httpRequest.getContextPath())
			.scheme(request.getScheme()).serverName(request.getServerName()).port(request.getServerPort())
