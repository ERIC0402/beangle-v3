...
public class ChineseSpelling {
	...
	public String getSelling(String chs) {
		...
		//	if (key.getBytes().length == 2) {//unicode编码中文不是双字节所以不应判断
				value = (String) convert(key);
				if (value == null||value.equals("?")) {
					//value = "unknown";
					//value = key;//无法转换拼音的字符转换为原来的字符
					//value="?";
					continue;
				}
		//	} else {
	...
	public static void main(String[] args) {
		...
		finder.setResource("简体中文-汉语");
		System.out.println(finder.getSpelling());
		System.out.println(finder.getSelling("英文字母-English"));
		System.out.println(finder.getSelling("日文假名-にほんこ"));
		System.out.println(finder.getSelling("繁体中文-漢語"));
