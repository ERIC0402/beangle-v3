...
Picasa.prototype = {
	...
	// 参数
	loadingImage	: base + '/static/scripts/Picasa/Picasa_Loading.gif',		// 加载中提示图片路径
	errorImage		: base + '/static/scripts/Picasa/Picasa_Error.gif',		// 加载错误提示图片路径
	...
	initialize : function() {
		...
		var count = anchors.length;
		var rel, info, img, name, hint;
		for (var i=0; i<count; i++){
			rel = String(anchors[i].getAttribute('rel'));
			if (rel.match(/picasa/i)){
				info = rel.replace(/picasa|[\[\s\]]/ig, '') +',';
				img = [anchors[i].getElementsByTagName('img')[0].src, info.split(',')[0]];
				name = img[1].substr(img[1].lastIndexOf('\/')+1);
				hint = info.split(',')[1];
				anchors[i].setAttribute('PicasaNo', this.arrItems.length);
				anchors[i].onclick = function(){
					self.display(this); 
					this.blur(); return false;}
				this.arrItems.push(new Array(img, name, [0, 0], hint));
			}
		}
	...
	createFrame : function() {
		...
		this.frame.innerHTML =
			'<img id="PV_Loading" src="'+ this.loadingImage +'" />'+
			'<img id="PV_Error" src="'+ this.errorImage +'" />'+
		...
		// 框架对象
		this.Picture	= $$('PV_Picture');				// 大图
		this.Loading	= $$('PV_Loading');				// 加载提示图
		this.Error		= $$('PV_Error');				// 错误提示图
		this.fPerHint	= $$('PV_PerHint');				// 缩放百分比提示框
		this.fHint		= $$('PV_Hint');					// 底部提示框
		this.fControl	= $$('PV_Control');				// 控制栏
		this.fItems		= $$('PV_Items');				// 列表项
		this.fButtons	= $$('PV_Buttons');				// 控制按钮

		// 按钮对象
		this.btnZoomIn		= $$('PV_Btn_ZoomIn');		// 放大(+)
		this.btnZoomOut		= $$('PV_Btn_ZoomOut');		// 缩小(-)
		this.btnZoomActual	= $$('PV_Btn_ZoomActual');	// 原始尺寸(1:1)
		this.btnPrev		= $$('PV_Btn_Prev');			// 上一张图片
		this.btnSlide		= $$('PV_Btn_Slide');		// 幻灯片演示
		this.btnNext		= $$('PV_Btn_Next');			// 下一张图片
		this.btnAutoPlay	= $$('PV_Btn_AutoPlay');		// 自动播放
		...
		this.activeImage.onload = function() {self.doLoad = setTimeout(function() {self.loadedAction();}, 200)}
		this.activeImage.onerror = function() {self.doLoad = setTimeout(function() {self.loadError();}, 200)}
	...
	createItems : function() {
		...
		for (var i=0; i<this.arrCount; i++){
			itemStr += '<li><a href="javascript: void(0);" onclick="'+ this.name +'.changeImage('+ i +'); return false;"><img src="'+ this.arrItems[i][0][0] +'" /></li>';
		}
	...
	setElementEvent : function() {
		...
		this.btnPrev.onmouseover = function() {self.hint(self.lng.prev);$$('PV_Select').className='PV_Btn_PrevOver';}
		this.btnPrev.onmouseout = function() {self.hint();$$('PV_Select').className='PV_Btn_Normal';}
		...
		this.btnSlide.onmouseover = function() {self.hint(self.lng.slide);$$('PV_Select').className='PV_Btn_SildeOver';}
		this.btnSlide.onmouseout = function() {self.hint();$$('PV_Select').className='PV_Btn_Normal';}
		this.btnSlide.onclick = function() {return false;}
	
		this.btnNext.onmouseover = function() {self.hint(self.lng.next);$$('PV_Select').className='PV_Btn_NextOver';}
		this.btnNext.onmouseout = function() {self.hint();$$('PV_Select').className='PV_Btn_Normal';}
	...
	loadImage : function() {
		...
		if(this.Loading.offsetLeft==0) {
			this.Loading.style.left = (this.initialCoord[0]-this.Loading.width/2) +'px';
			this.Loading.style.top = (this.initialCoord[1]-this.Loading.height/2) +'px';
		}
	...
	loadError : function() {
		...
		this.Error.style.left = (this.initialCoord[0]-this.Error.width/2) +'px';
		this.Error.style.top = (this.initialCoord[1]-this.Error.height/2) +'px';
	...
	hint : function() {
		...
		if(arguments[0]) {
			msg = arguments[0];
			//msg = String(arguments[0]).replace(/\s|<br?.+>/ig, '');
		}else{
			msg = this.hintTxt;
		}
	...
	perHint : function() {
		...
		/*
		this.fPerHint.innerHTML = arguments[0] +'%';
		if(this.doHidePerHint==null) {
			var self = this;
			clearInterval(this.doFadePerHint);
			this.doFadePerHint = this.fadeAction(1, self.fPerHint, this.perHintOpacity);
			this.doHidePerHint = setTimeout(function(){
				this.doFadePerHint = self.fadeAction(0, self.fPerHint, 0);
			}, 500);
		}else{
			clearTimeout(this.doHidePerHint);
			this.doHidePerHint = null;
		}
		*/
	},
...
MyPicasa.lng = language.chs;
//Event.add(window, 'load', new Function('MyPicasa.initialize()'));
