...
(function(window, undefined) {
	...
	beangle.extend({
		...
		Go : function(obj, target, ajaxHistory) {
			...
			//ajaxHistory = false;
//			if(undefined == ajaxHistory || null == ajaxHistory){
//				ajaxHistory = false;
//			}
			var url = obj;
	...
	beangle.extend({
		input : {
			...
			/**
			 * modify by beangle 2006-8-2 将反选取消,改为全选或者全部取消
			 */
			...
			.extend({
				form : {
					submit : function(myForm, action, target, onsubmit, ajax,
						...
						// disabled button 
						if($(myForm).find(".searchButton").length == 0){
							jQuery(myForm).find(":button,:submit,:reset").attr("disabled",true);
						}
						// fixed ckeditor
...
//						}
					...
					goToPage : function(form, pageNo, pageSize, orderBy) {
						if ((typeof form) != "object") {
							bg.alert("[goToPage:]form is not well defined.");
							return;
						}
	...
	jQuery.struts2_jquery
			.require(
					"/struts/js/plugins/jquery.form"
							...
							//"/static/metronic1.5.4/assets/plugins/jquery-validation/lib/jquery.form.js",
					function() {
								...
								jQuery(document).ready(function() {
													History.Adapter.bind(window, 'statechange', function(e) {
																		...
																		if (jQuery.type((currState.data || {}).target) != "undefined" && jQuery.type((currState.data || {}).html) != "undefined") {
																			...
																			if(History.savedStates.length > 20){
																				History.reset();
//																				History.store={};
////																				History.savedHashes=[];
////																				History.savedStates=[];
...
//																					
//																				}
//																				History.savedHashes.shift();
//																				History.savedStates.shift();
////																				History.idToState.shift();
////																				History.urlToId.shift();
////																				History.stateToId.shift();
//																				History.storedStates.shift();
																			}
																		}
																	});
												});
