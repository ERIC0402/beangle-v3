...
(function() {
	...
	function c(u) {
		...
		if (A) {
			for (D = 0; D < y; D++) {
				x = v[D].attributes;
				x.style
						&& (x.style = C([ [ 'list-style-type' ] ])(x.style)
								|| '');
			}
	...
	;
	var d = /^([.\d]*)+(em|ex|px|gd|rem|vw|vh|vm|ch|mm|cm|in|pt|pc|deg|rad|ms|s|hz|khz){1}?/i, e = /^(?:\b0[^\s]*\s*){1,4}$/, f = '^m{0,4}(cm|cd|d?c{0,3})(xc|xl|l?x{0,3})(ix|iv|v?i{0,3})$', g = new RegExp(
			f), h = new RegExp(f.toUpperCase()), i = {
		decimal : /\d+/,
		'lower-roman' : g,
		'upper-roman' : h,
		'lower-alpha' : /^[a-z]+$/,
		...
		ul : j
			...
			[ 9, 'IX' ], [ 5, 'V' ], [ 4, 'IV' ], [ 1, 'I' ] ], m = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
	function n(u) {
		u = u.toUpperCase();
		var v = l.length, w = 0;
		for ( var x = 0; x < v; ++x)
			for ( var y = l[x], z = y[1].length; u.substr(0, z) == y[1]; u = u
					.substr(z))
				w += y[0];
		return w;
	}
	...
	var p = 0, q = null, r, s = CKEDITOR.plugins.pastefromword = {
		utils : {
			...
			resolveList : function(u) {
				...
				if ((w = u.removeAnyChildWithName('cke:listbullet'))
						&& w.length && (w = w[0])) {
					u.name = 'cke:li';
					if (v.style)
						v.style = s.filters
		...
		filters : {
			flattenList : function(u, v) {
				...
				for ( var A = 0; A < y.length; A++) {
					...
					if (z.name in CKEDITOR.dtd.$listItem) {
						...
						y = y.slice(0, A).concat(z.children).concat(
								y.slice(A + 1));
						u.children = [];
			...
			elementMigrateFilter : function(u, v) {
				return function(w) {
					...
					w.name = x.element;
					CKEDITOR.tools.extend(w.attributes, CKEDITOR.tools
							.clone(x.attributes));
		...
		getRules : function(u) {
			var v = CKEDITOR.dtd, w = CKEDITOR.tools.extend({}, v.$block,
					v.$listItem, v.$tableContent), x = u.config, y = this.filters, z = y.falsyFilter, A = y.stylesFilter, B = y.elementMigrateFilter, C = CKEDITOR.tools
					.bind(this.filters.styleMigrateFilter, this.filters), D = this.utils.createListBulletMarker, E = y.flattenList, F = y.assembleList, G = this.utils.isListBulletIndicator, H = this.utils.isContainingOnlySpaces, I = this.utils.resolveList, J = function(
					O) {
			...
			return {
				...
				elements : {
					...
					style : function(O) {
						if (CKEDITOR.env.gecko) {
							var P = O.onlyChild().value
									.match(/\/\* Style Definitions \*\/([\s\S]*?)\/\*/), Q = P
									&& P[1], R = {};
							if (Q) {
								Q
										.replace(/[\n\r]/g, '')
								...
								y.applyStyleFilter = function(S) {
									var T = R['*'] ? '*' : S.name, U = S.attributes
											&& S.attributes['class'], V;
									if (T in R) {
										V = R[T];
										if (typeof V == 'object')
											V = V[U];
					...
					p : function(O) {
						if (/MsoListParagraph/.exec(O.attributes['class'])) {
							var P = O.firstChild(function(S) {
								return S.type == CKEDITOR.NODE_TEXT
										&& !H(S.parent);
					...
					div : function(O) {
						...
						if (P && P.name == 'table') {
							var Q = O.attributes;
							P.attributes = CKEDITOR.tools.extend(P.attributes,
									Q);
							...
							if (R) {
								...
								if (T
										&& / mso-hide:\s*all|display:\s*none /
												.test(T.attributes.style))
									S.attributes['cke:ignored'] = 1;
								return S;
							}
						}
						var U = O.children, V = O.attributes, W = V && V.style, X = U
								&& U[0];
						if (W)
							V.style = A(
									[
											...
											[
													/^font-size$/,
													null,
													!M ? C(x.fontSize_style,
															'size') : null ],
											[
													/^color$/,
													null,
													!M ? C(
															x.colorButton_foreStyle,
															'color')
															: null ],
											[
													/^background-color$/,
													null,
													!M ? C(
															x.colorButton_backStyle,
															'color')
															: null ] ])(W, O)
									|| '';
						return null;
					},
					b : B(x.coreStyles_bold),
					i : B(x.coreStyles_italic),
					u : B(x.coreStyles_underline),
					s : B(x.coreStyles_strike),
					sup : B(x.coreStyles_superscript),
					sub : B(x.coreStyles_subscript),
					a : function(O) {
						var P = O.attributes;
						if (P && !P.href && P.name)
							delete O.name;
						else if (CKEDITOR.env.webkit && P.href
								&& P.href.match(/file:\/\/\/[\S]+#/i))
							P.href = P.href.replace(/file:\/\/\/[^#]+/i, '');
					},
					'cke:listbullet' : function(O) {
						if (O.getAncestor(/h\d/)
								&& !x.pasteFromWordNumberedHeadingToList)
							delete O.name;
					}
				},
				attributeNames : [ [ /^onmouse(:?out|over)/, '' ],
						[ /^onload$/, '' ], [ /(?:v|o):\w+/, '' ],
						[ /^lang/, '' ] ],
				attributes : {
					style : A(
									...
									[
											...
											function(O, P, Q) {
												if (P.name in {
													...
													var R = x.contentsLangDirection == 'ltr' ? 'margin-left'
															: 'margin-right';
													if (Q == 'margin')
														O = K(Q, O, [ R ])[R];
													else if (Q != R)
				...
				comment : !CKEDITOR.env.ie ? function(O, P) {
					...
					if (CKEDITOR.env.gecko && Q) {
						var U = CKEDITOR.htmlParser.fragment.fromHtml(Q[0]).children[0], V = P.previous, W = V
								&& V.value
										.match(/<v:imagedata[^>]*o:href=['"](.*?)['"]/), X = W
								&& W[1];
