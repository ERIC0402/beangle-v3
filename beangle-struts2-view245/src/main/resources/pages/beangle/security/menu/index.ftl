	...
	<tr>
		<td class="index_view">
			[@b.form action="!search?orderBy=menu.code" title="ui.searchForm" target="menulist" theme="search"]
				[@b.select name="menu.profile.id" items=profiles?sort_by("id")?reverse label="配置"/]
		...
		[@b.div  href="!search?menu.profile.id=${(profiles?last.id)!}&orderBy=menu.code" id="menulist"/]
		</td>
	</tr>
</table>
[@b.foot/]
