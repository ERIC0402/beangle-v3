	...
	beangle.extend({
		...
		findTarget : function(ele) {
			...
			while (p) {
				// FIXME ui-tabs-panel
				if (p.id && p.className
						&& (p.className.indexOf("_ajax_target") > -1)) {// ||p.className.indexOf("ui-tabs-panel")>-1
					finalTarget = p.id;
					break;
				} else {
					if (p == p.parentNode)
			...
			.extend({
				form : {
					submit : function(myForm, action, target, onsubmit, ajax,
						...
						// disabled button 
						$(".listform").find(":button,:submit,:reset").attr("disabled",true);
												...
												function() {
													History.Adapter
															.bind(
																	...
																	'statechange',
																	function(e) {
																		var currState = History
																				.getState();
																		if (jQuery
																				.type((currState.data || {}).target) != "undefined"
																				&& jQuery
																						.type((currState.data || {}).html) != "undefined") {
																			jQuery(
																					currState.data.target)
																					.empty();
																			jQuery(
																					currState.data.target)
																					.html(
...
c.onprogress = function(msg) {
	...
	return c.dialogDiv = $("<div>"+msg+"</div>").dialog({
		title : '友情提示',
		show : 'fade',
		hide : 'fade',
		modal : true,
...
c.closeProgress = function (){
	c.dialogClosed = true;
	if(c.dialogDiv){
		c.dialogDiv.dialog("close");
		c.dialogDiv = null;
	}
}
