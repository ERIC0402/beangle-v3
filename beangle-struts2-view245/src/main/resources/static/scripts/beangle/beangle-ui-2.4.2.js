...
//			imageRoot=self.location.pathname.substring(0,self.location.pathname.substring(1).indexOf('/')+1)+"/static/themes/"+ bg.uitheme +"/icons/",
	...
	function EntityAction(entity,onePage){
		...
		this.openCurrent = function(methodName,paramName,paramValue,multiId,showUI,confirmMsg){
			window.$.unblockUI();
			return new NamedFunction(methodName,function(){
				try {
					...
					if (action.beforeSubmmitId(methodName, multiId)) {
						...
						action.submitIdAction(methodName, multiId, null,true);
					}
				}catch(e){
					bg.alert(e);
				}
	    ...
	    this.goTo = function(methodName,url){
			return new NamedFunction(methodName,function(){
				try {
					var form=action.getForm();
					form.target="main";
	    ...
	    this.openNewLayer = function(href0,width,height,checkboxId,isMulti,entityName,params){
		...
		this.method=function(methodName,confirmMsg,extparams,ajax){
			...
			return  new NamedFunction(methodName,function(){
				if(confirmMsg && confirmMsg.indexOf("dialog") == 0){
					try{c.onprogress(confirmMsg.substring(7))}catch(e){};
					confirmMsg = null;
				}
				var form=selfaction.getForm();
				if(null!=confirmMsg && ''!=confirmMsg){
				...
				bg.form.addInput(form, id + 's', selectId, "hidden");
