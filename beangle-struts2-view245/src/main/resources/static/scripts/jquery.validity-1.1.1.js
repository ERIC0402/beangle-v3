...
(function($) {
    ...
    $.validity = {
        ...
        patterns:{
            ...
            year:/^\d{4}$/,
            "month": /^[1-9]$|^1[012]$/,//请输入正确的月份！<br/>如：1-12
            "hour": /^0$|^(0)?[1-9]$|^1[0-9]$|^2[0-3]$/, //msg:"请输入小时数0-23"},
            "minute": /^0$|^(0)?[1-9]$|^[1-5][0-9]$/, //msg:"请输入分钟数0-59"},
...
//            phone:/^[2-9]\d{2}-\d{3}-\d{4}$/,
            phone: /^([\d]{3,4}-)?[\d]{6,12}(\*[\d]{1,4})?$/,
            "mphone": /^[\d]{11}$/,// 手机号
            "lxdh": /^(([\d]{3,4}-)?[\d]{6,12}(\*[\d]{1,4})?)$|^[\d]{11}$/,// 固定电话手机号
            guid:/^(\{?([0-9a-fA-F]){8}-(([0-9a-fA-F]){4}-){3}([0-9a-fA-F]){12}\}?)$/,

            nonHtml:/^[^<>]*$/,
            "ration":/^\d*\.?\d*\%?$/,// 可以是百分数
//            "phone":{reg:/^([\d]{3,4}-)?[\d]{6,12}(\*[\d]{1,4})?$/, msg:"电话号码格式不正确！<br>如：021-99999999*2222"},// 固定电话
            "sfz":/^[\d]{15}$|^[\d]{18}$|^[\d]{17}[X]$/, //msg:"身份证号码应为15、18位或者17位+X！"},// 身份证号
            "money": /^[\d]+(\.\d{1,2})?$/,//请输入正确的金额！如：12.98
            "hanzi": /^[\u4e00-\u9fa5]+$/ //msg:"只能输入汉字"}
        },
    ...
    $.fn.extend({
        ...
        // http://validity.thatscaptaintoyou.com/Demos/index.htm#LessThan
        maxl:function(max, msg) {
            return this.maxLength(max, msg);
        },
    ...
    function infer(field) {
        ...
        // Check for title.
        if ($f.attr("title") && $f.attr("title").length) {
            ret = $f.attr("title");
        }
