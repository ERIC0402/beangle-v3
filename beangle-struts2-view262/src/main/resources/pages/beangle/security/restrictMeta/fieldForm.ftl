...
[@b.form action="!saveField" title="数据限制参数" theme="list"]
	...
	[@b.select2 label="restriction.entities" name1st="Entitys" name2nd="entityId" items1st=entities items2nd=field.entities /]
	[@b.formfoot]
		<input type="hidden" name="field.id" value="${(field.id)!}"/>
		<input type="hidden" name="noTooltip" class="noTooltip"/>
		[@b.redirectParams/]
