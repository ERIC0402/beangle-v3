...
[@b.tabs]
	[@b.tab label="${b.text('ui.userInfo')}" theme="list" hasTable=1]
		...
		[/@]
		[#if isadmin|| isme]
		[@b.startend label="有效期" name="user.effectiveAt,user.invalidAt" required="true,false" start=user.effectiveAt end=user.invalidAt format="datetime" disabled="disabled"/]
		[#else]
		[@b.startend label="有效期" name="user.effectiveAt,user.invalidAt" required="true,false" start=user.effectiveAt end=user.invalidAt format="datetime"/]
		[/#if]
