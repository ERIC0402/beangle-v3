...
[#list data as v]
	[#if noOption]
		[#nested v/]
	[#else]
		<option value="${v[valueKey]}" title="${v[nameKey]!}" [#if value?string==v[valueKey]?string]selected=selected[/#if]>[@c.substring str=v[nameKey]!'' mx=20/]</option>
	[/#if]
[/#list]
</select>
[/#macro]
...
[#macro sjzt kssj jssj][#assign nkssj=dateToNumber(kssj)/][#assign njssj=dateToNumber(jssj)/][#assign nnow=dateToNumber(.now)/][#if nkssj != 0 && nkssj gt nnow]<font color="green">未开始</font>[#elseif nkssj lte nnow && (nnow lte njssj || njssj == 0)]<font color="red">进行中</font>[#else]已结束[/#if][/#macro]
