...
[@c.tr tag=tag]
${tag.parameters['beforeInput']!}<input type="text" id="${tag.id}" title="[#if tag.comment??]${tag.comment}[#else]${tag.title!}[/#if]" name="${tag.name}" value="${tag.value?html}" maxlength="${tag.maxlength}" ${tag.parameterString}/>
[/@]